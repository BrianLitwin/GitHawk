...
class ShortlinkRegExTests: XCTestCase {
    ...
    func test_positiveMatches() {
        var builder: StyledTextString = setupBuilder(with: "#1205")
        var containsLink = checkForIssueLink(builder.styledTexts)
        XCTAssertTrue(containsLink)
        
        builder = setupBuilder(with: "with a space preceding #1205")
        ...
        builder = setupBuilder(with: "embedded in parentheses (#181)")
        containsLink = checkForIssueLink(builder.styledTexts)
        XCTAssertTrue(containsLink)
        
        builder = setupBuilder(with: "with owner and repo preceding rnystrom/githawk#1792")
        containsLink = checkForIssueLink(builder.styledTexts)
        XCTAssertTrue(containsLink)
    }
    
    func test_negativeMatches() {
        var builder = setupBuilder(with: "!1900")
        var containsLink = checkForIssueLink(builder.styledTexts)
        ...
        builder = setupBuilder(with: "Fixes#1792")
        containsLink = checkForIssueLink(builder.styledTexts)
        XCTAssertFalse(containsLink)
        ...
        builder = setupBuilder(with: "#179T)")
        containsLink = checkForIssueLink(builder.styledTexts)
        XCTAssertFalse(containsLink)
    }
}
